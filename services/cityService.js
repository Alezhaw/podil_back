...
class CityService {
    ...
    async ChangeCheck(id, id_for_base, check_base, check_speaker, check_scenario, user, country) {
        ...
        const cities = await this.GetTimeByIdForBase(city.id_for_bas);

        const result = cities?.map((city) =>
            ObjectHelper.sendDifferencesToDatabase(city, { ...city.dataValues, ...CityService.checkValue(check_base, check_speaker, check_scenario) }, country, "update", user, "city")
        );
        if (!result[0]) {
            throw ApiError.internal("Failed to write log");
        }
        const updated = await this.Update(CityService.checkValue(check_base, check_speaker, check_scenario), { id_for_base: city.id_for_base }, country);

        const updatedCity = await this.GetTimes(city.id_for_base);
